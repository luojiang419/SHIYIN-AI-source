...
ASPECT_RATIOS = ("source", "1:1", "2:3", "3:2", "3:4", "4:3", "4:5", "5:4", "9:16", "16:9", "21:9")
...
SIZE_PRESETS: dict[str, dict[str, str]] = {
    ...
    "5:4": {"1k": "1280x1024", "2k": "2040x1632", "4k": "3200x2560"},
    "9:16": {"1k": "720x1280", "2k": "1152x2048", "4k": "2160x3840"},
    "16:9": {"1k": "1280x720", "2k": "2048x1152", "4k": "3840x2160"},
    "21:9": {"1k": "1344x576", "2k": "2016x864", "4k": "3360x1440"},
}
...
def build_prompt(operation: str, inputs: Iterable[dict[str, Any]], options: dict[str, Any] | None = None) -> str:
    ...
    if prompt_policy == LOOKBOOK_PROMPT_POLICY:
        ...
        wardrobe_lock = (
            "WARDROBE SCENE STYLING LOCK: the supplied people references provide identity, face, hair, body proportions and natural skin only; "
            "their photographed interview outfits are not to be copied into the final composite. Design a coherent scene-specific wardrobe for this campaign, "
            "with practical garments, denim/utility/swim layers appropriate to the location and weather. Keep the two people distinct and keep clothing continuity within the series; "
            "do not change wardrobe between frames without a story reason, and do not let styling alter identity."
            if str(options.get("lookbook_wardrobe_mode") or "").strip().lower() == "scene_styled"
            else "WARDROBE IMMUTABILITY LOCK: when a supplied 人物 reference visibly wears an outfit, that exact outfit is the styling source of truth. Preserve garment category, silhouette, neckline, closures, knit/weave, color, denim wash, footwear, bag and accessories at every frame; do not upgrade, restyle, replace or simplify the clothing merely because the brief says fashion editorial. Any change of clothing requires an explicit user instruction."
        )
        parts = [
        ...
        if effective_style_id in {"", "candid-lifestyle", "single-person-emotion", "casual-friends", "travel-dream", "pet-fashion"}:
            ...
            parts.extend([
                "2026 FW REFERENCE FILM LOCK: derive a repeatable fashion-film look from the supplied reference images, not from generic 'cinematic' adjectives. Use high-key natural sun or thin-cloud daylight with warm ivory, cream and pale lemon-yellow as the dominant 60-70% field; faded turquoise/cyan architecture or props as a controlled 20-30% complement; keep truthful warm-neutral skin and reserve any other hue for a small 5-10% accent sampled from the actual scene. Keep a small true black anchor in hair, deep interiors, rail gaps and hardware, separate the subject from the background with a deliberate luminance step, open the midtones without gray haze, protect white garments and faces from clipping, and roll highlights into a short creamy soft shoulder. High-key means luminous exposure with structured contrast, never a flat low-contrast wash, foggy milk veil, teal-orange blockbuster grade or random hue shift.",
                "FW TONAL CONTRAST LOCK: match the supplied FW references with a confident high-key structured S-curve—deep readable blacks, clearly separated warm skin/cream highlights, cyan shadow planes and a visible midtone step. Preserve texture in the shadow side but do not lift every shadow to the same gray; let one side of the face, dark hair or interior fall 1-2 stops deeper than the sunlit plane. Use controlled specular highlights and a restrained black point, not HDR crunch. For 16:9 outputs, build a horizontal luminance hierarchy: brightest subject/garment plane, darker framing or environment plane, and one clean negative-space area; never flatten the whole frame into one beige exposure.",
            ...
            parts.extend([
                ...
                "DENIM COLOR AND TONAL SCRIPT: derive the grade from the supplied scene rather than inventing a fixed brand palette. Keep approximately 60% environment and denim midtones, 25% truthful skin and neutral clothing, and 15% controlled accent sampled from the scene. Preserve indigo, washed blue, charcoal, off-white and muted earth colors; use red or another saturated accent only when visible or explicitly requested. Maintain open midtones, protected skin and fabric highlights, readable blacks and gentle highlight roll-off; do not flatten every scene into teal-orange or a beige wash.",
                "DOCUMENTARY CAMERA: use a human eye-level or waist-level handheld viewpoint with natural perspective. Build a series from a wide environmental establishing frame, a human-distance observed action frame, an intimate emotion frame and one tactile contact detail. Vary horizon, distance, crop and foreground occlusion with purpose; use doorways, windows, rails, vehicles, furniture, plants or garment edges as real framing elements. Keep feet grounded and match perspective, ambient color and contact shadows so the person belongs inside the supplied location.",
                "HUMAN PERFORMANCE AND GROOMING: give every frame one external objective that belongs to the supplied scene or references—arriving somewhere, handling a supplied object, garment, tool or product, reacting to an off-frame person or sound, paying, leaving, sheltering from weather or working at a real station. Direct the body in the middle of that task: eyes follow the objective, fingers form a functional grip, shoulders and hips counterbalance, weight transfers between feet and clothing responds to momentum. Emotion is the involuntary reaction inside the task, never a beauty pose; keep hands occupied only when the object or environment gives them a reason, and reject idle hair/chest/face touching. Preserve the supplied identity, face shape, skin tone, pores, hairline, hair texture, body proportions, visible makeup and wardrobe exactly. Use tactile strands, clean but human skin, minimal corrective makeup and weather-appropriate hair movement; do not invent a new face, freckles, scars, age cues or beauty filter.",
                "SET AND MATERIAL REALISM: keep only scene-owned furniture, street furniture, tools, plants, vehicles and practical lights, with no invented showroom. Preserve denim weave, seams, stitching, rivets, abrasion, folds, drape, skin pores, flyaway hair, dust, rain beads and physically correct contact shadows. Finish with restrained organic film texture, natural motion softness, gentle halation only on bright edges and low sharpening; reject plastic skin, CGI gloss, crushed blacks and crunchy HDR.",
                "SINGLE-FRAME AND ORIGINALITY LOCK: each output is one full-bleed photograph, never a grid, split screen, storyboard or collage. Across four separate outputs use environmental establish, observed action, emotional close view and tactile garment/product proof. Extract only transferable methods from the supplied advertising evidence; do not copy any source person, slogan, logo, exact location or distinctive composition.",
            ])
        ...
        if "人物" in semantic_roles and "场景" in semantic_roles:
            ...
            if str(options.get("lookbook_wardrobe_mode") or "").strip().lower() == "scene_styled":
                parts.append("PERSON IDENTITY-ONLY EDITORIAL LOCK: use each connected 人物 image only for that person's face, identity, skin tone, hair, body proportions and natural asymmetries. Ignore the photographed interview outfit and design a scene-specific wardrobe requested by the creative brief; keep the new wardrobe continuous across the series and never merge the two identities. Build a lived-in fashion editorial around concrete tasks, interaction and environmental depth rather than a blank studio pose.")
            else:
