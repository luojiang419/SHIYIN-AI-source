    ...
    *,
    ...
    if has_fabric_detail:
        ...
        fabric_rule = (
            ...
            "先判断特写是否真有毛羽，再观察并迁移实际可见的织物组织、织线方向与间距、纤维粗细、表面起伏、柔软度和光泽；"
            "若能看到连续斜纹、平纹或针织线圈，就在完整衣片上保持相同的组织方向与自然尺度，让纹理随衣片曲面和褶皱变化；"
            "只有特写确有绒毛时才迁移其长度、密度和方向；没有时不得生成绒面、雾状磨砂或随机噪点。"
            ...
            "不得用锐化噪点、硬边印花或塑料光泽代替真实织物，也不得把清楚的细织纹抹成平滑色块。"
            "输出一张完整照片，不输出细节拼贴、图号或文字。"
            f"\n【服装原色锁定，优先于环境融合与面料特写】\n图{garment_index}是最终服装唯一的颜色标准，"
            f"图{fabric_index}仅补充表面微观结构，不提供颜色、白平衡、曝光或对比度。"
            "分别锁定完整服装参考中面料底色和各纹样的可见原色，不能只保留相近的色系。"
            "把这些原色直接用于新衣的正常受光中间调，保持色相、饱和度、冷暖与明度关系一致。"
            "人物及街景保持目标图片原来的色调，但衣服必须独立匹配服装参考的白平衡；"
            "禁止暖光、环境反射、肤色、目标旧衣或整体电影调色给衣服染色。"
            "空间适配仅允许必要的遮挡暗部和很小范围的高光，不得整体提亮底色、压黑纹样、泛黄、偏粉或偏棕。"
            "输出前将服装与完整服装参考并排核对底色和纹样：若存在整体偏色，须先恢复参考原色，"
            "同时保留纤维与绒毛，不能以融入场景为理由接受颜色差异。"
        )
