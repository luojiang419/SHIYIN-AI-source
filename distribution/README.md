...
服务端为每次目录响应附加独立Ed25519签名的最终目标版本。用户首次确认更新后，客户端在 `data/update/catchup.json` 保存补齐计划；后续重启自动下载并应用下一步，直到已应用序号达到目标才清除。手动检查策略也会续跑已确认计划，明确关闭更新则暂停。软件完全关闭时不安装常驻代理，下一次启动后再自动补齐最新累积快照。
