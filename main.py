...
def require_film_bridge_loopback(request: Request) -> None:
    if not is_loopback_address(request_remote_address(request)):
...
@app.post("/api/canvas-bridges/film/receive")
async def receive_film_bridge_package(
    request: Request,
    file: UploadFile = File(...),
    canvas_id: str = Form(""),
    canvas_title: str = Form(""),
    ...
    """本机 filmstoryboard 自动发送入口：接收桥接包并直接持久化画布节点。"""
    require_film_bridge_loopback(request)
    suffix = Path(str(file.filename or "bridge.zip")).suffix.lower()
    if suffix not in {".zip", ".filmbridge", ".shiyinbridge"}:
        raise HTTPException(status_code=400, detail="桥接包扩展名不受支持。")
    max_upload_bytes = 2 * 1024 * 1024 * 1024
    temp_fd, temp_name = tempfile.mkstemp(prefix="film_bridge_receive_", suffix=suffix)
    os.close(temp_fd)
    extraction_root = tempfile.mkdtemp(prefix="film_bridge_receive_extract_")
    package = None
    try:
        total = 0
        with open(temp_name, "wb") as output:
        ...
        try:
            frame_records = await asyncio.to_thread(
                ...
                os.fspath(OUTPUT_INPUT_DIR),
                canvas["id"],
                selected_variant=str(storyboard.get("selected_variant") or ""),
            )
        except BridgePackageError as exc:
...
async def import_local_ai_reference(payload: LocalImageImportRequest, request: Request):
    ...
    requested = [p for p in requested if str(p or "").strip()]
    if not requested:
...
async def invoke_kling_cli_video(payload: CanvasVideoRequest, *, submit_only: bool = False):
    if any(str(value or "").strip() for value in (payload.videos or [])):
        raise HTTPException(
            status_code=400,
            detail="可灵视频参考已移除；视频截取功能仍可正常使用，片段可连接其他支持视频输入的平台。",
        )
    ...
    if provider_id == "kling-cli":
        if int((task.get("request") or {}).get("video_count") or 0) > 0:
            return {
                ...
                "url": "",
                "error": "旧版可灵视频参考任务已停止支持，不会继续调用开放平台 API。",
                "raw": None,
...
_VIDEO_DIRECTOR_EXPANSION_RULES = (
    ...
    "人物要有眼神先行、眨眼、视线转移和身体重心变化；仅当画面或用户语境确实存在动物或拟动物时，才依据其物种线索、姿态、环境和故事目标选择嗅闻、甩耳、警觉、打滚、侧躺、伸懒腰、踢地、追随或避让等行为，示例只是可选动作库，绝不能套用成固定套路；拟人化物体也要根据材质、形态和受力方式决定惯性、停顿、回弹和环境接触反馈。"
    "这是内容无关的导演决策，不得预设牛仔裤、牧场、动物或任何示例主体；先识别当前参考图与用户描述的真实语境，再选择最合理且有创意的动作、反应、节奏和镜头。没有充分视觉或叙事依据时，宁可保持克制，不强行添加示例动作。"
...
# 自动解析专用的动作密度约束。此前模型即使遵守“主动作 + 运镜”，仍可能把
# 每个镜头压缩成一两句静态结果描述；这里把可执行动作拆成时间节拍和因果链，
# 让输出明确“如何开始、如何变化、如何收住”，同时不把具体示例主体硬编码进规则。
_VIDEO_ACTION_CHOREOGRAPHY_REQUIREMENTS = (
    "动作编排硬性要求：最终提示词必须写出可在时间轴上执行的动作节拍，而不是只写主体‘站着、看着、很紧张’等静态状态。"
    "每个镜头至少包含 3 个连续微节拍：准备/蓄势（重心、预备动作或注意力变化）→启动与加速/减速（明确身体部位、方向、速度或力度）→互动、碰撞或空间关系变化（说明谁触发谁、接触点和物理反馈）→反应与收束（余势、停顿、回望、呼吸、姿态落点）；时长不足时合并相邻节拍，但不能省略因果关系。"
    "用‘先…随后…紧接着…最后…’或明确的时间区间表达节拍，避免只用一个笼统动词概括整段运动；每个主动作都要有起始构图、运动中构图和结束构图，写清状态如何改变并被下一镜头承接。"
    "多主体必须编排主次和错峰：先指定触发者，再写延迟观察、跟随、躲避、反扑或波纹式扩散的主体反应，延迟应以半拍/约数帧/短暂停顿等可感知方式表达；禁止整齐同步地重复同一动作。"
    "镜头不能只写‘轻微跟随/静态保持/展示动态’。运镜要对动作做出响应（例如在启动时加速跟进、在冲撞前侧移让位、在反应时短暂停住或绕到另一侧），并说明运动结束时揭示的空间信息或情绪变化；除非用户明确要求，禁止把‘static/静止机位’作为整镜头方案。"
    "每个镜头至少加入一项由动作触发的环境或材质反馈（尘土、草叶、水花、布料/毛发惯性、阴影、脚步接地、道具回弹等，按当前画面选择），并让声音与具体节拍对齐。避免抽象形容词堆砌，优先使用可观察的动词、身体部位、方向、速度、力度、接触和结果。"
)
