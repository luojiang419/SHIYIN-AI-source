...
2026-09-16：该引擎成为当前画布唯一入口。节点业务模板仍由 `canvas.js` 提供，挂载、增量更新、离屏复用、删除和清空统一由引擎负责；内容及直接连接依赖变化才重建节点。离屏 DOM 最多缓存 192 个，活动输入和播放节点保持挂载，全景节点离屏释放 WebGL。图片直接设置缩略图 `src`，当前页面不再使用旧媒体队列、驻留回收或 LOD。历史 smart-canvas 文件保留兼容用途，不参与当前画布渲染。
