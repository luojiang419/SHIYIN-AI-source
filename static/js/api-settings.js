...
function scheduleProviderAutoSave(delay=520){
    ...
    providerAutoSaveTimer = setTimeout(() => {
        ...
        saveProviders({render:false, auto:true});
    }, Math.max(0, Number(delay) || 0));
}
function saveKeyByKind(kind){
    if(kind === 'free') return saveRhKeyOnly('free');
    if(kind === 'wallet') return saveRhKeyOnly('wallet');
    if(kind === 'volcengine') return saveVolcengineAssetKeys();
    return saveKeyOnly();
}
...
function updateIdPreview(){
    const item = provider();
    if(!item) return;
    const isBuiltin = item.id === 'comfly' || item.id === 'modelscope' || item.id === 'runninghub' || item.id === 'volcengine' || item.id === 'grsai' || item.id === 'lingjing' || item.id === 'jimeng' || item.id === 'codex' || item.id === 'local-vision' || item.id === 'minimax-h3';
                ...
                <div class="onboarding-rh-linear-rows">
                    <div class="onboarding-rh-linear-row onboarding-ms-linear-row">
                        ...
                        <label class="onboarding-key-field onboarding-rh-row-field">
                            <span>API Key</span>
                            <input type="password" data-autosave-key="standard" value="${escapeAttr(keyInput?.value || '')}" placeholder="${escapeAttr(tr('api.msTokenPlaceholder'))}" oninput="syncOnboardingKeyInput('standard', this.value)">
                        </label>
                    </div>
                </div>
            </div>
                ...
                <div class="onboarding-rh-linear-rows">
                    <div class="onboarding-rh-linear-row">
                        ...
                        <label class="onboarding-key-field onboarding-rh-row-field">
                            ...
                            <input type="password" data-autosave-key="free" value="${escapeAttr(rhFreeKeyInput?.value || '')}" placeholder="${escapeAttr(tr('api.rhCoinPlaceholder'))}" oninput="syncOnboardingKeyInput('free', this.value)">
                        </label>
                    </div>
                    <div class="onboarding-rh-linear-row">
                        <div class="onboarding-rh-source-group">
                        ...
                        <label class="onboarding-key-field onboarding-rh-row-field">
                            ...
                            <input type="password" data-autosave-key="wallet" value="${escapeAttr(rhWalletKeyInput?.value || '')}" placeholder="${escapeAttr(tr('api.rhWalletPlaceholder'))}" oninput="syncOnboardingKeyInput('wallet', this.value)">
                        </label>
                    </div>
                </div>
            </div>
            ...
            <div class="onboarding-step-panel onboarding-rh-linear-panel onboarding-ms-linear-panel">
                ...
                <div class="onboarding-rh-linear-rows">
                    <div class="onboarding-rh-linear-row onboarding-ms-linear-row">
                        ...
                        <label class="onboarding-key-field onboarding-rh-row-field">
                            <span>API Key</span>
                            <input type="password" data-autosave-key="standard" value="${escapeAttr(keyInput?.value || '')}" placeholder="${escapeAttr(tr('api.lingjingKeyPlaceholder'))}" oninput="syncOnboardingKeyInput('standard', this.value)">
                        </label>
                    </div>
                </div>
...
async function saveOnboardingRunningHubKey(){
    ...
    syncEditor();
    const ok = await saveProviders({render:false, auto:true});
    if(ok){
...
function syncEditor(){
    ...
    const selectedProtocol = item.id === 'modelscope' || item.id === 'grsai' || item.id === 'local-vision' ? 'openai' : item.id === 'runninghub' ? 'runninghub' : item.id === 'volcengine' ? 'volcengine' : item.id === 'jimeng' ? 'jimeng' : item.id === 'codex' ? 'codex' : item.id === 'minimax-h3' ? 'minimax-h3' : (protocolInput?.value || 'openai');
...
function updateProtocolFromInput(){
    const item = provider();
    if(!item || !protocolInput || item.id === 'modelscope' || item.id === 'runninghub' || item.id === 'volcengine' || item.id === 'grsai' || item.id === 'jimeng' || item.id === 'codex' || item.id === 'minimax-h3') return;
...
function createRhEntryFromPaste(){
    ...
    if(!exists) scheduleProviderAutoSave(0);
}
function updateRhEntry(kind, index, prop, value){
    ...
    if(prop === 'title' || prop === 'note') scheduleProviderAutoSave();
}
...
function renderEditor(){
    ...
    if(protocolInput){
        protocolInput.value = item.id === 'runninghub' ? 'runninghub' : item.id === 'volcengine' ? 'volcengine' : item.id === 'jimeng' ? 'jimeng' : item.id === 'codex' ? 'codex' : item.id === 'minimax-h3' ? 'minimax-h3' : item.id === 'grsai' || item.id === 'local-vision' ? 'openai' : (item.protocol || 'openai');
...
let jimengLoginPolling = false;
let apiSettingsRouteActive = window.top === window;
function stopJimengLoginPolling(){
    clearInterval(jimengLoginTimer);
    jimengLoginTimer = null;
}
function resumeJimengLoginPolling(){
    stopJimengLoginPolling();
    if(!apiSettingsRouteActive || !jimengLoginPolling) return;
    pollJimengLogin();
...
async function startJimengLogin(){
    ...
    try {
        ...
        jimengLoginPolling = true;
        resumeJimengLoginPolling();
        refreshIcons();
    } catch(e){
...
function applyModelPicker(){
    ...
    renderMsLoras();
    setStatus(`已应用 · 生图 ${image.length} / LLM ${chat.length} / 视频 ${video.length}`);
    scheduleProviderAutoSave(0);
    closeModelPicker();
}
...
const FIXED_PROTOCOL_PROVIDER_IDS = new Set(['modelscope', 'volcengine', 'jimeng', 'runninghub', 'codex', 'local-vision', 'minimax-h3']);
...
function saveProviders(options={}){
    const run = () => persistProviders(options);
    providerSaveChain = providerSaveChain.then(run, run);
...
async function persistProviders(options={}){
    ...
    providers.forEach(item => {
        ...
        item.protocol = item.id === 'runninghub'
            ...
            : item.id === 'minimax-h3'
            ? 'minimax-h3'
            : ['openai', 'apimart', 'gemini', 'volcengine', 'runninghub', 'jimeng', 'codex', 'minimax-h3'].includes(String(item.protocol || '').toLowerCase()) ? String(item.protocol).toLowerCase() : 'openai';
        item.image_request_mode = normalizeImageRequestMode(
            item.id === 'modelscope' || item.id === 'runninghub' || item.id === 'volcengine' || item.id === 'grsai' || item.id === 'jimeng' || item.id === 'codex' || item.id === 'local-vision'
                ? 'openai'
    ...
    try {
        ...
        providers = providers.map(item => {
            ...
            return {
                ...
                has_wallet_key:saved.has_wallet_key,
                wallet_key_preview:saved.wallet_key_preview,
                has_volcengine_access_key:saved.has_volcengine_access_key,
                volcengine_access_key_preview:saved.volcengine_access_key_preview,
...
window.addEventListener('message', event => {
    if(event.data?.type === 'studio-route-active'){
        apiSettingsRouteActive = Boolean(event.data.active);
        if(apiSettingsRouteActive) resumeJimengLoginPolling();
        else stopJimengLoginPolling();
    }
...
window.onload = () => {
    ...
    keyInput?.addEventListener('change', () => { if(keyInput.value.trim()) saveKeyOnly(); });
    rhFreeKeyInput?.addEventListener('change', () => { if(rhFreeKeyInput.value.trim()) saveRhKeyOnly('free'); });
    rhWalletKeyInput?.addEventListener('change', () => { if(rhWalletKeyInput.value.trim()) saveRhKeyOnly('wallet'); });
    [volcAkInput, volcSkInput].forEach(input => input?.addEventListener('change', () => {
        if((volcAkInput?.value || volcSkInput?.value || '').trim()) saveVolcengineAssetKeys();
    }));
    settingsContent?.addEventListener('input', event => {
        const input = event.target.closest('input,textarea');
        if(!input || input.type === 'password' || input.type === 'file' || input === rhPasteInput) return;
    ...
    settingsContent?.addEventListener('change', event => {
        ...
        if(!control) return;
        if(control.dataset.autosaveKey) {
            if(control.value.trim()) saveKeyByKind(control.dataset.autosaveKey);
            return;
        }
