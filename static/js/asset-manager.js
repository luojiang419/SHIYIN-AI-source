...
let openAssetClassGroup = 'product';
...
let openLocalUploadClassGroup = 'product';
...
function slugifyReferenceType(value=''){
    return String(value || '').trim().toLowerCase().replace(/[^a-z0-9_-]+/g, '_').replace(/^_+|_+$/g, '').slice(0, 48);
}
function referenceTypeLabel(item){
    return item?.label_zh || item?.name || item?.id || '参考类型';
}
function normalizeReferenceSlotTypes(list){
    return (Array.isArray(list) ? list : []).filter(Boolean).map((item,index) => ({
        id:slugifyReferenceType(item.id || item.role || `type_${index + 1}`) || `type_${index + 1}`,
        role:slugifyReferenceType(item.role || item.id || 'prop') || 'prop',
        label_zh:String(item.label_zh || item.name || item.id || '参考类型').trim().slice(0, 48),
        label_en:String(item.label_en || item.label_zh || item.name || item.id || 'Reference').trim().slice(0, 64),
        enabled:item.enabled !== false,
        locked:item.locked === true,
        order:Number(item.order ?? index * 10),
    })).sort((a,b) => a.order - b.order);
}
function referenceTypeRoleOptions(selected='prop'){
    return REFERENCE_TYPE_ROLE_OPTIONS.map(([id,label]) => `<option value="${escapeAttr(id)}" ${id === selected ? 'selected' : ''}>${escapeHtml(label)}</option>`).join('');
}
function referenceTypeSelectOptions(selected=''){
    return referenceSlotTypes.filter(item => item.enabled !== false).map(item => `<option value="${escapeAttr(item.id)}" ${item.id === selected ? 'selected' : ''}>${escapeHtml(referenceTypeLabel(item))}</option>`).join('');
}
...
function assetLibraries(){
    return Array.isArray(assetLibrary.libraries) && assetLibrary.libraries.length
        ...
        : [{id:'default', name:'服装电商素材库', categories:assetLibrary.categories || []}];
}
...
function workflowCategories(){
    ...
    assetLibraries().forEach(lib => {
        (lib.categories || []).filter(cat => (cat.type || '') === 'workflow').forEach(cat => {
            cats.push({...cat, __libraryId:lib.id, __libraryName:lib.name || '素材库'});
        });
    });
    return cats;
}
...
function assetViewTitle(){
    ...
    return activeAssetCategory()?.name || '服装素材';
}
function assetViewSubtitle(items){
    const libName = activeAssetLibrary()?.name || '素材库';
    const entry = activeAssetClassEntry();
...
const ASSET_CLASS_GROUPS = [
    {id:'product', name:'商品', dims:['garment_category','garment_type','subject','objects']},
    {id:'shot', name:'拍摄类型', dims:['shot_type','view','composition']},
    {id:'model', name:'模特/穿搭', dims:['model','people','fit','body_part','styling']},
    {id:'fabric', name:'面料/细节', dims:['fabric','materials','craft','pattern','color']},
    {id:'commerce', name:'电商用途', dims:['use_case','channel','pdp_slot','readiness','rights']},
    {id:'season', name:'季节/场景', dims:['season','occasion','scene','environment','space','mood']},
    {id:'quality', name:'质量', dims:['quality','background','style','lighting','tags']},
];
...
function renderLocalCaptionTools(imageCount){
    ...
    return `
        <div class="local-caption-tools">
            ...
            ${localClassifyPromptOpen ? `<textarea id="localClassifyPrompt" class="local-classify-prompt-input" placeholder="默认已内置服装品类、具体款式、拍摄类型、模特/穿搭、面料工艺、季节场景、渠道状态等分类。这里可以追加你的分类要求，例如：增加平台、SKU、系列、尺码段、价格带、上新批次。">${escapeHtml(localClassifyPrompt || '')}</textarea>` : ''}
        </div>
    `;
}
...
function assetMoveTargets(){
    ...
    assetLibraries().forEach(lib => {
        (lib.categories || []).filter(cat => (cat.type || 'image') === 'image').forEach(cat => {
            ...
            if(key !== currentKey) targets.push({key, libraryId:lib.id, categoryId:cat.id, label:`${lib.name || '素材库'} / ${cat.name || '分组'}`});
        });
    });
    return targets;
}
...
function render(){
    ...
    else if(activeTab === 'reference-types') renderReferenceTypesManager();
...
function renderReferenceTypesManager(){
    referenceSlotTypes = normalizeReferenceSlotTypes(referenceSlotTypes);
    if(!selectedReferenceTypeId && referenceSlotTypes.length) selectedReferenceTypeId = referenceSlotTypes[0].id;
    if(selectedReferenceTypeId && !referenceSlotTypes.some(item => item.id === selectedReferenceTypeId)) selectedReferenceTypeId = referenceSlotTypes[0]?.id || '';
    const active = referenceSlotTypes.find(item => item.id === selectedReferenceTypeId) || null;
    root.innerHTML = `
        <aside class="asset-panel asset-nav">
            <div class="panel-head"><div class="panel-title"><strong>参考类型</strong><span>自由换衣与全能模式共用</span></div></div>
            <div class="nav-scroll">
                <div class="nav-tree">
                    ${referenceSlotTypes.map(item => `<button class="tree-row tree-parent ${item.id === selectedReferenceTypeId ? 'active' : ''}" type="button" data-reference-type-select="${escapeAttr(item.id)}">
                        <span class="tree-row-icon"><i data-lucide="${item.enabled === false ? 'eye-off' : 'tag'}"></i></span>
                        <span class="tree-row-name">${escapeHtml(referenceTypeLabel(item))}</span>
                        <span class="tree-row-count">${escapeHtml(REFERENCE_TYPE_ROLE_OPTIONS.find(([id]) => id === item.role)?.[1] || item.role)}</span>
                    </button>`).join('')}
                </div>
                <div class="nav-hint">类型名称是给用户看的标签；生成时会按“生成角色”映射到稳定的模型提示词。</div>
            </div>
        </aside>
        <section class="asset-panel asset-content">
            <div class="content-toolbar">
                <div class="content-heading">
                    <strong>全局参考图格子类型</strong>
                    <span>${referenceSlotTypes.filter(item => item.enabled !== false).length}/${referenceSlotTypes.length} 个已启用</span>
                </div>
                <div class="asset-tools">
                    <select id="referenceTypePreviewSelect" class="manage-select" title="预览下拉选项">${referenceTypeSelectOptions(selectedReferenceTypeId)}</select>
                    <button class="asset-btn" type="button" data-reference-type-add><i data-lucide="plus"></i><span>新增类型</span></button>
                    <button class="asset-btn primary" type="button" data-reference-type-save><i data-lucide="save"></i><span>保存修改</span></button>
                </div>
            </div>
            <div class="content-scroll">
                <div class="reference-type-editor">
                    ${referenceSlotTypes.map((item,index) => renderReferenceTypeRow(item,index)).join('')}
                </div>
            </div>
        </section>
        <aside class="asset-panel asset-detail">
            ${renderReferenceTypeDetail(active)}
        </aside>
    `;
}

function renderReferenceTypeRow(item, index){
    return `<article class="reference-type-row" data-reference-type-row="${escapeAttr(item.id)}">
        <div class="reference-type-row-head">
            <strong>${escapeHtml(String(index + 1).padStart(2, '0'))}</strong>
            <div class="asset-tools compact-actions">
                <button class="asset-icon-btn" type="button" data-reference-type-move="${escapeAttr(item.id)}" data-direction="-1" ${index <= 0 ? 'disabled' : ''} title="上移"><i data-lucide="arrow-up"></i></button>
                <button class="asset-icon-btn" type="button" data-reference-type-move="${escapeAttr(item.id)}" data-direction="1" ${index >= referenceSlotTypes.length - 1 ? 'disabled' : ''} title="下移"><i data-lucide="arrow-down"></i></button>
                <button class="asset-icon-btn danger" type="button" data-reference-type-delete="${escapeAttr(item.id)}" ${item.locked ? 'disabled' : ''} title="${item.locked ? '内置类型不能删除' : '删除'}"><i data-lucide="trash-2"></i></button>
            </div>
        </div>
        <div class="reference-type-grid">
            <label><span>类型 ID</span><input data-reference-field="id" value="${escapeAttr(item.id)}" ${item.locked ? 'readonly' : ''}></label>
            <label><span>生成角色</span><select data-reference-field="role">${referenceTypeRoleOptions(item.role)}</select></label>
            <label><span>中文标签</span><input data-reference-field="label_zh" value="${escapeAttr(item.label_zh)}"></label>
            <label><span>英文标签</span><input data-reference-field="label_en" value="${escapeAttr(item.label_en)}"></label>
        </div>
        <label class="reference-type-enabled"><input type="checkbox" data-reference-field="enabled" ${item.enabled !== false ? 'checked' : ''}> <span>在下拉选项中启用</span></label>
        <input type="hidden" data-reference-field="locked" value="${item.locked ? '1' : '0'}">
    </article>`;
}

function renderReferenceTypeDetail(item){
    if(!item) return `<div class="panel-head"><div class="panel-title"><strong>类型详情</strong><span>暂无参考类型</span></div></div><div class="detail-scroll"><div class="detail-empty"><i data-lucide="tags"></i><span>暂无参考类型</span></div></div>`;
    const roleLabel = REFERENCE_TYPE_ROLE_OPTIONS.find(([id]) => id === item.role)?.[1] || item.role;
    return `
        <div class="panel-head"><div class="panel-title"><strong>类型详情</strong><span>${escapeHtml(item.id)}</span></div></div>
        <div class="detail-scroll">
            <div class="detail-body">
                <div class="detail-name">${escapeHtml(referenceTypeLabel(item))}</div>
                <div class="detail-meta-grid">
                    <div class="detail-meta"><span>生成角色</span><strong>${escapeHtml(roleLabel)}</strong></div>
                    <div class="detail-meta"><span>状态</span><strong>${item.enabled === false ? '已停用' : '已启用'}</strong></div>
                    <div class="detail-meta"><span>来源</span><strong>${item.locked ? '内置' : '自定义'}</strong></div>
                    <div class="detail-meta"><span>排序</span><strong>${Number(item.order || 0)}</strong></div>
                </div>
                <div class="nav-hint">自由换衣拖拽排序会决定提交生成时的参考图顺序。</div>
            </div>
        </div>`;
}

function readReferenceTypesFromDom(){
    return [...root.querySelectorAll('[data-reference-type-row]')].map((row,index) => {
        const field = name => row.querySelector(`[data-reference-field="${name}"]`);
        const id = slugifyReferenceType(field('id')?.value || row.dataset.referenceTypeRow || `type_${index + 1}`);
        return {
            id,
            role:slugifyReferenceType(field('role')?.value || 'prop') || 'prop',
            label_zh:String(field('label_zh')?.value || id || '参考类型').trim(),
            label_en:String(field('label_en')?.value || field('label_zh')?.value || id || 'Reference').trim(),
            enabled:field('enabled')?.checked !== false,
            locked:field('locked')?.value === '1',
            order:index * 10,
        };
    });
}

async function saveReferenceSlotTypesFromDom(){
    const types = normalizeReferenceSlotTypes(readReferenceTypesFromDom());
    const data = await apiJson('/api/reference-slot-types', {
        method:'PUT',
        headers:{'Content-Type':'application/json'},
        body:JSON.stringify({types}),
    });
    referenceSlotTypes = normalizeReferenceSlotTypes(data.types);
    setStatus('参考类型已保存');
    render();
}
...
function addReferenceSlotType(){
    ...
    referenceSlotTypes.push({id, role:'prop', label_zh:'新参考类型', label_en:'New reference type', enabled:true, locked:false, order:referenceSlotTypes.length * 10});
    selectedReferenceTypeId = id;
    render();
}

function moveReferenceSlotType(id, direction){
    const index = referenceSlotTypes.findIndex(item => item.id === id);
    const next = index + Number(direction || 0);
    if(index < 0 || next < 0 || next >= referenceSlotTypes.length) return;
    const copy = referenceSlotTypes.slice();
    copy.splice(next, 0, copy.splice(index, 1)[0]);
    referenceSlotTypes = copy.map((item, order) => ({...item, order:order * 10}));
    selectedReferenceTypeId = id;
    render();
}

function deleteReferenceSlotType(id){
    const item = referenceSlotTypes.find(value => value.id === id);
    if(!item || item.locked) return;
    referenceSlotTypes = referenceSlotTypes.filter(value => value.id !== id).map((value, index) => ({...value, order:index * 10}));
    selectedReferenceTypeId = referenceSlotTypes[0]?.id || '';
    render();
}
...
function renderLocalClipboardBar(){
    ...
    return `<div class="asset-clipboard-bar">
        <div class="asset-clipboard-info"><i data-lucide="clipboard"></i><span>${escapeHtml(modeLabel)}了 ${localClipboard.items.length} 个本地素材，目标：${escapeHtml(activeAssetLibrary()?.name || '服装素材')} / ${escapeHtml(target?.name || '未选择分组')}</span></div>
        <div class="asset-tools">
            <button class="asset-btn primary" type="button" data-local-import-clipboard ${target ? '' : 'disabled'}><i data-lucide="clipboard-paste"></i><span>导入到服装素材</span></button>
...
function renderAssetManager(){
    ...
    root.innerHTML = `
        <aside class="asset-panel asset-nav">
            <div class="panel-head">
                <div class="panel-title"><strong>服装素材层级</strong><span>按商品、拍摄类型、渠道组织</span></div>
                <div class="panel-actions compact-actions">
                    ${assetTreeEdit?.placement === 'head' ? '' : '<button class="asset-icon-btn" type="button" data-asset-lib-new title="新建素材库"><i data-lucide="plus"></i></button>'}
        ...
        <section class="asset-panel asset-content ${assetManageMode ? 'manage-on' : ''}">
            <div class="content-toolbar">
                ...
                <div class="asset-tools">
                    <label class="asset-search-wrap"><i data-lucide="search"></i><input id="assetSearch" class="asset-search" type="search" value="${escapeAttr(assetQuery)}" placeholder="搜索 SKU / 款式 / 颜色 / 场景"></label>
            ...
            <div class="content-scroll">
                <div class="asset-grid">
                    ...
                    ${items.length ? '' : '<div class="empty-state">当前分组还没有服装素材，可以上传商品图、模特图、平铺图或细节图。</div>'}
                </div>
            </div>
        </section>
        <aside class="asset-panel asset-detail">
...
function renderWorkflowManager(){
    ...
    root.innerHTML = `
        ...
        <section class="asset-panel asset-content ${workflowManageMode ? 'manage-on' : ''}">
            <div class="content-toolbar">
                <div class="content-heading">
                    <strong>${escapeHtml(cat?.name || '素材管理')}</strong>
...
function renderUploadCard(cat){
    return `<button id="assetDrop" class="upload-grid-card" type="button" data-asset-upload ${!cat ? 'disabled' : ''}>
        <span class="upload-thumb"><i data-lucide="upload-cloud"></i></span>
        <span class="upload-body">
            <strong>上传服装素材</strong>
            <small>商品图、模特图、平铺图、细节图</small>
        </span>
    </button>`;
}
...
function renderAssetTreeBranch(lib){
    ...
    return `<div class="tree-branch ${isActiveLib ? 'expanded' : ''}">
        <button class="tree-row tree-parent ${isActiveLib ? 'contains-active' : ''} ${showLibActions ? 'active' : ''}" type="button" data-asset-lib="${escapeAttr(lib.id)}">
            ...
            <span class="tree-row-name">${escapeHtml(lib.name || '素材库')}</span>
        ...
        <div class="tree-children">
            ${cats.length ? cats.map(cat => `<button class="tree-row tree-child ${isActiveLib && cat.id === activeAssetCategoryId && assetTreeFocus === 'category' ? 'active' : ''}" type="button" data-asset-cat="${escapeAttr(cat.id)}" data-asset-cat-lib="${escapeAttr(lib.id)}">
                <span class="tree-elbow"></span>
                <span class="tree-row-icon"><i data-lucide="shirt"></i></span>
                <span class="tree-row-name">${escapeHtml(cat.name || '服装素材')}</span>
        ...
        <div class="detail-scroll">
            ...
            <div class="detail-body">
                ...
                <div class="detail-meta-grid">
                    <div class="detail-meta"><span>类型</span><strong>${escapeHtml(assetKindLabel(item))}</strong></div>
                    <div class="detail-meta"><span>创建时间</span><strong>${escapeHtml(formatDate(item.created_at))}</strong></div>
                    <div class="detail-meta"><span>素材库</span><strong>${escapeHtml(activeAssetLibrary()?.name || '素材库')}</strong></div>
...
async function handleClick(event){
    ...
    if(activeTab === 'reference-types'){
        const referenceSelect = target.closest?.('[data-reference-type-select]');
        if(referenceSelect){
            selectedReferenceTypeId = referenceSelect.dataset.referenceTypeSelect || '';
            render();
            return;
        }
        if(target.closest?.('[data-reference-type-add]')){
            addReferenceSlotType();
            return;
        }
        if(target.closest?.('[data-reference-type-save]')){
            try {
                await saveReferenceSlotTypesFromDom();
            } catch(err) {
                setStatus(err.message || '保存参考类型失败');
            }
            return;
        }
        const referenceMove = target.closest?.('[data-reference-type-move]');
        if(referenceMove){
            moveReferenceSlotType(referenceMove.dataset.referenceTypeMove || '', Number(referenceMove.dataset.direction || 0));
            return;
        }
        const referenceDelete = target.closest?.('[data-reference-type-delete]');
        if(referenceDelete){
            deleteReferenceSlotType(referenceDelete.dataset.referenceTypeDelete || '');
            return;
        }
    }
    const tabBtn = target.closest?.('[data-tab]');
    if(tabBtn){
        const nextTab = tabBtn.dataset.tab || 'assets';
        activeTab = nextTab === 'workflows' ? 'assets' : nextTab;
        selectedAssetIds.clear();
        selectedWorkflowIds.clear();
        selectedPromptIds.clear();
        selectedLocalIds.clear();
    ...
    if(target.closest?.('[data-asset-lib-new]')){ assetTreeFocus = 'library'; assetTreeEdit = {kind:'library-new', placement:'head', value:'服装电商素材库', label:'素材库名称'}; render(); focusTreeEditInput('assetTreeEditInput'); return; }
    if(target.closest?.('[data-asset-lib-rename]')){
        ...
        assetTreeEdit = {kind:'library-rename', value:activeAssetLibrary()?.name || '', label:'素材库名称'};
        pendingTreeDelete = '';
        render(); return;
    }
    ...
    if(target.closest?.('[data-asset-cat-new]')){
        ...
        assetTreeEdit = {kind:'category-new', value:'商品主图/白底图', label:'服装素材分组名称'};
        pendingTreeDelete = '';
        render(); return;
    }
    if(target.closest?.('[data-asset-cat-rename]')){
        ...
        assetTreeEdit = {kind:'category-rename', value:activeAssetCategory()?.name || '', label:'服装素材分组名称'};
        pendingTreeDelete = '';
        render(); return;
    }
...
async function createAssetLibrary(){
    const name = window.prompt('素材库名称', '服装电商素材库');
    if(!String(name || '').trim()) return;
...
async function createAssetCategory(){
    const name = window.prompt('服装素材分组名称', '商品主图/白底图');
    if(!String(name || '').trim()) return;
...
async function renameAssetCategory(){
    const cat = activeAssetCategory();
    const name = window.prompt('服装素材分组名称', cat?.name || '');
...
function setLocalClipboard(mode, ids=null){
    ...
    pendingBatchDelete = '';
    render();
    setStatus(`复制了 ${items.length} 个共享素材，导入后会拷贝到服装素材分组（共享文件夹原文件保留）`);
}
async function pasteLocalClipboardToAssets(){
    if(!localClipboard?.items?.length) return;
    if(!activeAssetCategory()){
        setStatus('请先在服装素材中创建或选择分组');
        return;
    }
    ...
    selectedLocalId = '';
    render();
    setStatus(`已导入 ${imported} 个素材到服装素材`);
}
...
document.querySelectorAll('[data-tab]').forEach(btn => {
    btn.addEventListener('click', () => {
        const nextTab = btn.dataset.tab || 'assets';
        activeTab = nextTab === 'workflows' ? 'assets' : nextTab;
        selectedAssetIds.clear();
        selectedWorkflowIds.clear();
        selectedPromptIds.clear();
