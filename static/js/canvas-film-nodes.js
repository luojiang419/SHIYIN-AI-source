(function(){
    ...
    function normalize(node){
        ...
        if(node.type === 'film-storyboard'){
            ...
            node.storyboardGrain = Number.isFinite(Number(node.storyboardGrain)) ? Math.round(Math.max(0,Math.min(10,Number(node.storyboardGrain)))) : 0;
    ...
    function resetVideoPromptTask(node, currentPrompt){
        const current=String(currentPrompt || '');
        const previous=node.videoPromptTaskResult;
        const source=previous && current===previous.result ? previous.source : current;
    ...
    function bind(root,node,options={}){
        ...
        if(polishButton && prompt && options.polishPrompt){
            ...
            polishButton.addEventListener('click',async event=>{
                ...
                const taskPrompt=[String(source || '').trim(), submissionExternalPromptText(node, options)].filter(Boolean).join('\n\n');
        ...
        root.querySelectorAll('[data-film-field]').forEach(control=>{
            ...
            control.addEventListener(eventName,event=>{
                ...
                if(key==='storyboardGrain'){
                    control.value=node.storyboardGrain;
                    const output=root.querySelector('[data-film-grain-value]');
                    if(output) output.textContent=String(node.storyboardGrain);
                }
                if(key==='apiProvider'){
                ...
                } else if(node.type === 'film-video' && previousRule.id === 'minimax'){
                    node.duration=clamp(node.duration,1,60);
                    node.resolution='1080p';
        ...
        root.querySelectorAll('[data-film-toggle]').forEach(button=>{
            button.addEventListener('mousedown',event=>event.stopPropagation());
            button.addEventListener('click',event=>{
                event.preventDefault(); event.stopPropagation();
                const field=button.dataset.filmToggle;
