(function(){
    ...
    function summary(node, assets){
        try {
            ...
            return node.storyboardMode==='batch' ? `已识别 ${items.length} 个镜头 · 每镜 1 张 · 共用演员与服化道` : `单图合成 · 生成 ${items.length} 张`;
    ...
    function createPreparer(node, options={}){
        ...
        return async plan=>{
            let assets=plan.assets.map(ref=>({...ref}));
            ...
            built.refs=built.refs.map(ref=>({...ref,role:ref.inputRole==='depth' ? 'control_map' : ref.role,
                role_label:ref.inputRole==='depth' ? '深度图：只控制模特姿势与空间结构' : ref.inputRole==='reference' ? '原始参考图：只参考构图色彩光线，背景从零重建' : ref.role_label}));
            built.prompt=[built.prompt,REALISM,assets.some(ref=>ref.role==='reference') ? REBUILD : '依照线稿/深度图重建真实立体空间，使用指定场景的材质与光线；没有色彩参考时采用自然且物理合理的摄影光线。','只输出当前镜头的一张完整照片，禁止拼贴、多格分镜、线稿、灰度深度图、文字编号和水印。'].filter(Boolean).join('\n');
            return built;
        };
    }
