...
function classicFilmInheritedAssets(node){
    ...
    storyboardNodes.forEach(storyboard => {
        connections.filter(connection => connection.to === storyboard.id && /^(actor|outfit|prop)-\d+$/.test(connection.inputRole || '')).forEach(connection => {
            const inputRole = String(connection.inputRole);
            const source = nodes.find(item => item.id === connection.from);
            mediaRefsFromNode(source).forEach(ref => inherited.push({
                ref,
                role:inputRole,
                inputRole,
...
function classicFilmInheritedActorAssets(node){
    // 兼容旧调用名；影视资产现在按演员、服装、道具整组复用。
    return classicFilmInheritedAssets(node);
}
function syncClassicFilmAutoReuse(){
    nodes.filter(node => node.type === 'film-video').forEach(node => {
        const inherited = classicFilmInheritedAssets(node);
...
function classicFilmAssets(node){
    ...
    const allInherited = classicFilmInheritedAssets(node);
...
const FLUX2_KLEIN_4B_LINE_ART_PROMPT = 'Edit the supplied reference by replacing every visible person with a uniform blank articulated storyboard mannequin. Remove all hair, facial features, ears, beard, makeup, skin detail, wedding dress, suit, jewelry, shoes, fabric seams, folds, and costume detail. Fill every removed human appearance with the same neutral mannequin construction: a smooth egg-shaped head with a completely blank face, plain neck, simple geometric chest and pelvis, visible round shoulder, elbow, wrist, hip, knee, and ankle joints, cylindrical arms and legs, and block-like hands and feet. Preserve exactly only the source canvas and crop, camera viewpoint, perspective, environment geometry, person count, figure position, figure scale, body pose, head direction, hand gesture, contact points, overlap, and essential held props. Draw a single borderless black-and-white production storyboard on white paper with clean thin graphite-and-ink lines and sparse structural hatching. Keep blank areas blank.';
...
function arrangeCanvasLayerItems(items, rectById, startX, startY){
    ...
    ordered.forEach((item, index) => {
        moveCanvasNodeAtom(item, colX[index % columns], rowY[Math.floor(index / columns)]);
    });
    return {
...
function arrangeIdsByConnections(ids){
    ...
    [...layers.keys()].sort((a, b) => a - b).forEach(level => {
        ...
        let layerY = startY;
        const layerLayout = arrangeCanvasLayerItems(items, rectById, layerX, startY);
