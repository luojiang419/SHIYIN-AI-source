...
const SMART_HISTORY_MEDIA_LIMIT = 48;
...
function disableLocalComfySettings(target){
    if(!target || typeof target !== 'object') return target;
    if(String(target.engine || '').toLowerCase() === 'comfy'){
        target.engine = 'api';
        target.apiKind = 'image';
        target.localGenerationDisabled = true;
    }
    return target;
}
function settingsForStorage(source=settings){
    const clean = cloneSmartSettings(source);
    disableLocalComfySettings(clean);
    delete clean.comfyWorkflow;
    delete clean.comfy_workflow;
    delete clean.comfyParams;
    delete clean.comfy_params;
    delete clean.comfyMode;
    delete clean.comfyRandomActive;
...
function normalizeSmartVideoModeSettings(target, preferMultimodal=false){
    if(!target || typeof target !== 'object') return target;
    disableLocalComfySettings(target);
...
const MULTI_VIEW_INPUT_SLOTS = window.CanvasBuildingMultiView?.PERSON_INPUT_SLOTS || [
    ...
    ['product-lower-front', '下装正面', true], ['product-lower-side', '下装侧面', true], ['product-lower-back', '下装背面', true],
    ['front-detail', '正面细节', true], ['back-detail', '背面细节', true], ['accessory', '配饰', true]
];
const BUILDING_MULTI_VIEW_INPUT_SLOTS = window.CanvasBuildingMultiView?.BUILDING_INPUT_SLOTS || [
...
const ALL_MULTI_VIEW_INPUT_SLOTS = [...MULTI_VIEW_INPUT_SLOTS, ...BUILDING_MULTI_VIEW_INPUT_SLOTS];
function smartMultiViewMode(node){
    return window.CanvasBuildingMultiView?.mode(node) || (node?.multiViewMode === 'building' ? 'building' : 'person');
}
function smartMultiViewInputSlots(node){
    return smartMultiViewMode(node) === 'building' ? BUILDING_MULTI_VIEW_INPUT_SLOTS : MULTI_VIEW_INPUT_SLOTS;
}
function normalizeSmartMultiViewNode(node){
    window.CanvasBuildingMultiView?.normalizeNode(node);
    if(node && !node.multiViewMode) node.multiViewMode = 'person';
    return node;
}
const MULTI_VIEW_LEGACY_ROLE_ALIASES = new Map([
...
function multiViewRoleTargets(role){
    if(ALL_MULTI_VIEW_INPUT_SLOTS.some(item => item[0] === role)) return [role];
    return MULTI_VIEW_LEGACY_ROLE_ALIASES.get(role) || [];
}
function migrateSmartMultiViewConnections(){
    ...
    canvas.connections.forEach(connection => {
        ...
        const roles = target?.specialType === 'multi-view' && (connection.kind || 'flow') === 'input'
            ? MULTI_VIEW_LEGACY_ROLE_ALIASES.get(connection.inputRole || '')
            : null;
        if(!roles?.length){ next.push(connection); return; }
        changed = true;
        roles.forEach((role, index) => next.push({...connection, ...(index ? {id:uid('c')} : {}), inputRole:role}));
    });
    if(changed) canvas.connections = next;
    return changed;
}
function multiViewRoleRatio(role){
    const resolvedRole = multiViewRoleTargets(role)[0] || role;
    const slots = resolvedRole.startsWith('building-') ? BUILDING_MULTI_VIEW_INPUT_SLOTS : MULTI_VIEW_INPUT_SLOTS;
    const index = slots.findIndex(item => item[0] === resolvedRole);
    return index >= 0 ? (index + 1) / (slots.length + 1) : 0.5;
}
function multiViewConnections(node){
    const result = new Map(smartMultiViewInputSlots(node).map(([role]) => [role, []]));
    (canvas?.connections || []).filter(conn => conn.to === node?.id && (conn.kind || 'flow') === 'input').forEach(conn => {
        const roles = multiViewRoleTargets(conn.inputRole || '');
        const source = nodes.find(item => item.id === conn.from);
        if(!source) return;
        imagesForNode(source).filter(item => item?.url && mediaKindForItem(item) === 'image').forEach(item => {
            roles.forEach(role => result.get(role)?.push(item));
        });
    });
    return result;
}
...
function multiViewBodyHtml(node){
    ...
    const outputNames = buildingMode ? ['线稿图', '建筑正面', '建筑侧视图', '建筑背视图', '建筑顶视图'] : ['纯三视图横板', '独立细节图', '正面视图', '侧面视图', '背面视图'];
    ...
    const slots = inputSlots.map(([role, label, kind], index) => {
        const connectionCount = (canvas?.connections || []).filter(connection => connection.to === node.id && connection.inputRole === role && (connection.kind || 'flow') === 'input').length;
        const count = kind === 'prompt' ? connectionCount : (inputs.get(role)?.length || 0);
        const detail = count ? (kind === 'prompt' ? '提示词已连接' : `${count} 张已连接`) : '可选输入';
        return `<div class="multi-view-slot-row" data-input-role="${escapeAttr(role)}" data-port-index="${index}"><span><small class="multi-view-slot-group">${escapeHtml(groupLabel(role))}</small><i data-lucide="${count ? 'circle-check' : 'circle-dashed'}"></i><strong>${escapeHtml(label)}</strong></span><b class="${count ? 'has-input' : ''}">${escapeHtml(detail)}</b></div>`;
    }).join('');
    ...
    return `<div class="special-node multi-view-special" data-special-node="multi-view">
        <div class="multi-view-summary">${summary}</div>
...
function invalidateSmartGeometry(ids=[]){
    ...
    list.forEach(id => smartNodeRectIndex.delete(id));
...
function cachedSmartNodeRect(node){
    ...
    const cached = smartNodeRectIndex.get(node.id);
    if(cached && cached.x === (Number(node.x) || 0) && cached.y === (Number(node.y) || 0)) return cached;
    const measured = nodeRect(node);
    smartNodeRectIndex.set(node.id, measured);
    return measured;
}
function cachedSmartPortPoint(node, kind, role='', rect=null){
    const key = smartPortIndexKey(node?.id || '', kind, role);
    const cached = smartPortGeometryIndex.get(key);
    if(cached?.epoch === smartGeometryEpoch) return cached;
    const r = rect || cachedSmartNodeRect(node);
    const isHistory = kind === 'history-out' || kind === 'history-in';
    const isOut = kind === 'out' || kind === 'history-out';
    const ratio = node?.specialType === 'multi-view' ? multiViewRoleRatio(role) : role === 'pose-reference' ? 0.36 : role === 'target-image' ? 0.68 : 0.5;
    const point = isHistory
        ? {x:r.x + r.width / 2, y:isOut ? r.y + r.height : r.y}
        : {x:isOut ? r.x + r.width : r.x, y:r.y + r.height * (isOut ? 0.5 : ratio)};
    const value = {...point, epoch:smartGeometryEpoch};
    smartPortGeometryIndex.set(key, value);
    return value;
}
...
function fitAllNodesViewport(){
    ...
    const rects = nodes.map(cachedSmartNodeRect);
...
function renderComfyParams(){
    ...
    dynamicParams.innerHTML = `
        <div class="smart-control comfy-mode-control">
            <button class="smart-pill" type="button"><i data-lucide="workflow"></i><span>${escapeHtml(modeOptions.find(([v]) => v === settings.comfyMode)?.[1] || '本地生成已停用')}</span></button>
            <div class="smart-popover compact-popover">
...
function assetLibraries(){
    return Array.isArray(assetLibrary.libraries) && assetLibrary.libraries.length ? assetLibrary.libraries : [{id:'default', name:'服装电商素材库', categories:assetLibrary.categories || []}];
}
...
function applyMergedServerCanvas(serverCanvas){
    ...
    const migratedMultiViewConnections = migrateSmartMultiViewConnections();
...
async function checkCanvasMetaOnce(){
    if(!smartRouteActive || document.hidden || !canvasId || !canvas) return;
...
function stopCanvasMetaPoll(){
    if(canvasMetaPollTimer) clearInterval(canvasMetaPollTimer);
    canvasMetaPollTimer = null;
}
function startCanvasMetaPoll(){
    // WS / iframe 转发不可靠时的兜底：仅当前路由激活时检查服务器版本。
    if(!smartRouteActive || document.hidden || canvasMetaPollTimer) return;
    canvasMetaPollTimer = setInterval(checkCanvasMetaOnce, 8000);
}
function pauseSmartRouteMedia(){
...
function resumeSmartRouteMedia(){
...
function setSmartRouteActive(active){
    ...
    if(!smartRouteActive || document.hidden){
        ...
        if(connectionLayerRaf){
            cancelAnimationFrame(connectionLayerRaf);
            connectionLayerRaf = 0;
        }
        if(interactionLayerRaf){
            cancelAnimationFrame(interactionLayerRaf);
            interactionLayerRaf = 0;
        }
        return;
    }
    resumeSmartRouteMedia();
    startCanvasMetaPoll();
    checkCanvasMetaOnce();
    refreshRunTimerPills();
    if(panoramaState.enabled && imageEditModal?.classList.contains('open')) startPanoramaLoop();
}
...
function renderAssetLibrary(){
    ...
    if(assetTab !== 'image') assetTab = 'image';
    ...
    if(assetLibrarySelect){
        assetLibrarySelect.innerHTML = libs.map(lib => `<option value="${escapeHtml(lib.id)}" ${lib.id === activeAssetLibraryId ? 'selected' : ''}>${escapeHtml(lib.name || '素材库')}</option>`).join('');
    }
...
async function loadCanvas(){
    ...
    try {
        ...
        let prunedRuntimeCollections = false;
        const loadedLogs = Array.isArray(canvas.logs) ? canvas.logs : [];
        if(loadedLogs.length > SMART_GENERATION_LOG_LIMIT) prunedRuntimeCollections = true;
        canvas.logs = loadedLogs.slice(0, SMART_GENERATION_LOG_LIMIT);
        nodes.forEach(node => {
            const before = Array.isArray(node.images) ? node.images.length : 0;
            node.images = isHistoryGroupNode(node)
                ? boundedSmartHistoryImages(node.images || [])
                : boundedSmartNodeImages(node.images || []);
            if(node.images.length !== before) prunedRuntimeCollections = true;
        });
...
async function saveCanvas(){
    ...
    try {
        const serializeEnd = window.CanvasPerformance?.start?.('smart.save.serialize', {nodes:nodes.length, connections:(canvas.connections || []).length});
        ...
        if(res.ok){
            ...
            saveRequestedWhileInFlight = true;
            trailingSaveDelay = Math.min(trailingSaveDelay, 300);
        }
    } catch(e) {} finally {
        canvasSyncInFlight = false;
        if(saveRequestedWhileInFlight){
            const delay = trailingSaveDelay;
            saveRequestedWhileInFlight = false;
            trailingSaveDelay = 450;
            scheduleSave(delay);
        }
...
function renderConnections(nodeIndex=new Map(nodes.map(node => [node.id, node]))){
    ...
    nodes.forEach(node => {
        if(!isSmartGroupNode(node)) return;
        groupScopeByNodeId.set(node.id, node.id);
        (node.items || []).forEach(id => {
            if(!groupScopeByNodeId.has(id)) groupScopeByNodeId.set(id, node.id);
        });
    });
    conns.forEach(conn => {
        const kind = conn.kind || 'flow';
        const targetNode = nodeIndex.get(conn.to);
        if(targetNode?.specialType === 'multi-view' && !smartMultiViewInputSlots(targetNode).some(([role]) => role === (conn.inputRole || ''))) return;
        const fromScope = kind === 'history' ? '' : (groupScopeByNodeId.get(conn.from) || '');
        const toScope = kind === 'history' ? '' : (groupScopeByNodeId.get(conn.to) || '');
    ...
    const paths = items.map(item => {
        const fromNode = nodeIndex.get(item.from);
        const toNode = nodeIndex.get(item.toId);
        ...
        const isPendingLine = !isCascade && item.targets.some(t => nodeIndex.get(t)?.pending);
        ...
        const fromPoint = cachedSmartPortPoint(fromNode, isHistory ? 'history-out' : 'out', '', fr);
        const toPoint = cachedSmartPortPoint(toNode, isHistory ? 'history-in' : 'in', item.inputRole || '', tr);
        const fx = fromPoint.x, fy = fromPoint.y;
        const tx = toPoint.x, ty = toPoint.y;
...
function updateNodeElementDuringResize(node){
    if(!node) return;
    const el = smartNodeDomIndex.get(node.id);
    if(!el){
        render();
        return;
    }
...
function smartRunPlatformLabel(run){
    const s = run?.settings || {};
    if(s.engine === 'comfy') return '本地生成已停用';
...
function smartRunRequestMeta(run){
    const s = run?.settings || {};
    if(s.engine === 'comfy') return {backend:'local-generation-disabled'};
...
function addSmartGenerationLog({run, outputs=[], runMs=0, error=''}) {
    ...
    canvas.logs = [entry, ...canvas.logs].slice(0, SMART_GENERATION_LOG_LIMIT);
...
function refreshRunTimerPills(){
    ...
    document.querySelectorAll('[data-run-timer]').forEach(el => {
        const node = nodeIndex.get(el.dataset.runTimer);
...
function rememberInlineVideoActivations(nodeIndex=new Map(nodes.map(node => [node.id, node]))){
    world.querySelectorAll('.image-node [data-image-index] video[data-inline-video-active="1"]').forEach(video => {
        ...
        const node = nodeIndex.get(nodeEl?.dataset.id);
...
function render(){
    ...
    tpl.content.querySelectorAll('.image-node[data-id]').forEach(fresh => {
        renderedNodeEls.set(fresh.dataset.id, fresh);
    });
...
function registerSmartCanvasPerfFixture(){
    window.CanvasPerformance?.registerFixtureFactory?.('smart', options => {
        ...
        canvas.connections = Array.from({length:edgeCount}, (_, index) => {
            ...
            return {id:`perf-smart-connection-${index}`, from:`perf-smart-${from}`, to:`perf-smart-${to}`, kind:'flow'};
        });
        selectedId = '';
        selectedIds = [];
        render();
        return () => {
            ...
            canvas.connections = previous.connections;
...
function measureSmartNodeImages(nodeIndex=new Map(nodes.map(node => [node.id, node])), root=world){
    (root || world).querySelectorAll?.('.image-node img,.image-node video').forEach(imgEl => {
        ...
        const node = nodeIndex.get(targetNodeId);
        const image = node?.images?.[index];
...
function clearPortDragVisual(){
    ...
    if(portDragState?.activePortEl) portDragState.activePortEl.classList.remove('is-active');
    if(portDragState?.activeNodeEl) portDragState.activeNodeEl.classList.remove('port-hover');
}
...
function updatePortDragVisual(){
    ...
    const perfEnd = window.CanvasPerformance?.start?.('smart.updatePortDragVisual');
    const fromNode = smartNodeIndex.get(portDragState.fromId);
    if(!fromNode){ perfEnd?.({skipped:true}); return; }
    const fr = cachedSmartNodeRect(fromNode);
    ...
    if(portDragState.activePortEl) portDragState.activePortEl.classList.remove('is-active');
    if(portDragState.activeNodeEl) portDragState.activeNodeEl.classList.remove('port-hover');
    portDragState.activePortEl = null;
    portDragState.activeNodeEl = null;
    if(portDragState.hoverTargetId){
        const targetNodeEl = smartNodeDomIndex.get(portDragState.hoverTargetId);
        ...
        const targetPortEl = smartPortDomIndex.get(smartPortIndexKey(portDragState.hoverTargetId, portDragState.hoverPort, portDragState.hoverRole)) || targetNodeEl?.querySelector(`.node-port[data-port="${portDragState.hoverPort}"]${roleSelector}`);
        targetPortEl?.classList.add('is-active');
        portDragState.activeNodeEl = targetNodeEl || null;
        portDragState.activePortEl = targetPortEl || null;
    }
    perfEnd?.();
}
...
function multiViewInputRefs(node){
    ...
    const products = Object.fromEntries(MULTI_VIEW_INPUT_SLOTS
...
function multiViewReferencePlan(view, refs, outputKind='view'){
    ...
    const push = (item, role, instruction) => {
        if(!item?.url) return;
        const label = multiViewRoleLabel(role);
...
function bindSmartSpecialNode(el, node){
    ...
    if(node?.specialType === 'multi-view'){
        el.querySelectorAll('[data-multi-view-mode]').forEach(button => button.addEventListener('click', event => {
            ...
            if(smartMultiViewMode(node) === nextMode || window.CanvasBuildingMultiView?.isBusy(node)) return;
            pushUndo();
        ...
        [provider, model, resolution, quality, buildingPrompt].filter(Boolean).forEach(control => {
            control.addEventListener('mousedown', event => event.stopPropagation());
            control.addEventListener('click', event => event.stopPropagation());
        });
        const updateSettings = (patch, rerender=false) => {
    ...
    elements.forEach(el => {
        ...
        el.querySelectorAll('.smart-video-play').forEach(btn => {
            ...
            btn.addEventListener('click', e => {
                ...
                const targetNodeId = item?.dataset.refNodeId || id;
                const imageIndex = Number(item?.dataset.refImageIndex ?? item?.dataset.imageIndex ?? 0);
                const owner = nodeIndex.get(targetNodeId);
...
function startPanoramaLoop(){
    if(!smartRouteActive || document.hidden) return;
...
function appendImagesToSmartNode(uploaded, targetId='', opts={}){
    ...
    if(!node){
        ...
        undoSuppressed = true;
        node = createImageNodeAt(center, [], {deferRender:true, deferSave:true, skipUndo:true, select:false});
        undoSuppressed = false;
    }
...
function connectInputNode(fromId, toId, inputRole=''){
    ...
    if(to.specialType === 'multi-view'){
        ...
        if(window.CanvasBuildingMultiView?.roleKind(inputRole) === 'prompt'){
            const hasPrompt = from.type === 'smart-prompt' || (isSmartGroupNode(from) && promptTextItemsForNode(from).some(Boolean));
...
function renderMentionPicker(source){
    ...
    const librarySelect = (mentionSource === 'asset' && assetLibs.length)
        ? `<label class="mention-library-row"><span>${escapeHtml(tr('smart.assetLibrary'))}</span><select class="mention-library-select" data-mention-library>${assetLibs.map(lib => `<option value="${escapeHtml(lib.id)}" ${lib.id === activeAssetLibraryId ? 'selected' : ''}>${escapeHtml(lib.name || '素材库')}</option>`).join('')}</select></label>`
        : '';
...
function boundedSmartNodeImages(images=[]){
    const clean = cleanHistoryImages(images);
    return clean.length > SMART_NODE_MEDIA_LIMIT ? clean.slice(-SMART_NODE_MEDIA_LIMIT) : clean;
}
function boundedSmartHistoryImages(images=[]){
    return cleanHistoryImages(images).slice(0, SMART_HISTORY_MEDIA_LIMIT);
}
...
function replaceOutputsToNodeWithHistory(node, additions, kind='image', meta=null, options={}){
    ...
    if(history){
        const archived = boundedSmartHistoryImages([...existing, ...(history.images || [])]);
...
function appendOutputsToNode(node, additions, kind='image', options={}){
    ...
    if(!next.length) return [];
    node.images = boundedSmartNodeImages([...existing, ...next]);
    markSmartNodeComplete(node);
    node.outputKind = kind;
...
function appendLoopOutputsToNode(node, additions, kind='image', ctx=smartLoopContext){
    ...
    if(initialized && !initialized.has(node.id)){
        ...
        if(existing.length){
            ...
            history.images = boundedSmartHistoryImages([...existing, ...(history.images || [])]);
            history.title = '历史分组';
            history.outputKind = kind;
            history.scale = MEDIA_GROUP_DEFAULT_SCALE;
            delete history.w;
...
function processSmartPortDragMove(e){
    if(!portDragState || !e) return;
    const perfEnd = window.CanvasPerformance?.start?.('smart.portDragMove');
    const p = screenToWorld(e);
...
function scheduleSmartPortDragMove(e){
    portDragLatestEvent = e;
    if(portDragMoveRaf) return;
    portDragMoveRaf = requestAnimationFrame(() => {
        portDragMoveRaf = 0;
        const latest = portDragLatestEvent;
        portDragLatestEvent = null;
        processSmartPortDragMove(latest);
    });
}
window.onmousemove = e => {
    ...
    if(resizeState){
        const node = smartNodeIndex.get(resizeState.id);
        if(!node) return;
    ...
    if(!dragState) return;
    const node = smartNodeIndex.get(dragState.id);
    if(!node) return;
...
window.addEventListener('message', event => {
    ...
    if(event.data?.type === 'studio-route-active') setSmartRouteActive(event.data.active);
...
document.addEventListener('visibilitychange', () => setSmartRouteActive(smartRouteActive));
