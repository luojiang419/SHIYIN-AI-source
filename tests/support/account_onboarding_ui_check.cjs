...
(async () => {
  ...
  try {
    ...
    const panel = await page.locator('.startup-onboarding-panel').boundingBox();
    assert.equal(panel.width, 680); assert.equal(panel.height, 640);
    assert.equal(await page.locator('.startup-onboarding-step').count(), 4);
