...
(async()=>{
    ...
    try {
        ...
        await editor.fill('');await page.getByRole('button',{name:'应用并返回',exact:true}).click();
        assert.equal(await editor.isVisible(),true);
        assert.equal(await page.evaluate(()=>nodes.find(n=>n.id==='pose').poseReplicatePromptTemplates['depth:base-wardrobe']),custom);
        await editor.fill(custom);await page.getByRole('button',{name:'应用并返回',exact:true}).click();
