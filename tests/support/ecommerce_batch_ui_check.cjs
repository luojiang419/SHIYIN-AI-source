...
(async()=>{
    ...
    try {
        ...
        await page.route('**/api/canvas-image-tasks/*', route => {
            ...
            const pollCount = (taskPollCounts.get(id) || 0) + 1;
            taskPollCounts.set(id, pollCount);
            if(pollCount <= 2) return route.fulfill({json:{task_id:id,status:'running'}});
        ...
        await page.waitForFunction(() => EcommerceBatchOutfit.snapshot().groups[0].pending_works.length === 2);
        assert.equal(await page.locator('.ec-batch-work-thumb-pending').count(), 2);
        assert.equal(await page.locator('.ec-batch-work-pending').count(), 1);
        assert.match((await page.locator('.ec-batch-works-shell > header > strong').textContent()).trim(), /生成中 2/);
        assert.equal(await page.locator('[data-batch-download-selected]').isDisabled(), true);
        assert.equal(await page.evaluate(() => {
            const spinner = document.querySelector('.ec-batch-work-pending .ec-spinner');
            return spinner ? getComputedStyle(spinner).animationName : '';
        }), 'ec-spin');
        if(screenshotPath) await page.screenshot({path:screenshotPath.replace(/(\.[^.]+)$/, '-pending$1'), fullPage:true});
        ...
        assert.match(await page.locator('#batchOutfitPreviewTargetImage').getAttribute('src'), /fixture\.png\?upload=/);
        assert.match(await page.locator('#batchOutfitPreviewImage').getAttribute('src'), /^data:image\/svg\+xml/);
        assert.equal(await page.locator('#batchOutfitPreviewCompareHandle').isDisabled(), false);
        const compareMediaBox = await page.locator('#batchOutfitPreviewMedia').boundingBox();
        const compareHandleBox = await page.locator('#batchOutfitPreviewCompareHandle').boundingBox();
        await page.mouse.move(compareHandleBox.x + compareHandleBox.width / 2, compareHandleBox.y + compareHandleBox.height / 2);
        await page.mouse.down();
        await page.mouse.move(compareMediaBox.x + compareMediaBox.width * .7, compareHandleBox.y + compareHandleBox.height / 2, {steps:4});
        await page.mouse.up();
        assert.ok(Math.abs(Number(await page.locator('#batchOutfitPreviewCompareHandle').getAttribute('aria-valuenow')) - 70) <= 1);
        ...
        const synchronizedTransforms = await page.locator('#batchOutfitPreviewMedia img').evaluateAll(images => images.map(image => getComputedStyle(image).transform));
        assert.equal(new Set(synchronizedTransforms).size, 1);
        ...
        assert.equal(await page.locator('#batchOutfitPreviewCompareHandle').getAttribute('aria-valuenow'), '50');
        assert.equal(await page.locator('[data-batch-preview-step="1"]').isDisabled(), true);
