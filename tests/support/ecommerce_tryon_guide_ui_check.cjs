...
(async () => {
    ...
    try {
        ...
        assert.equal(await page.locator('.ec-tryon-generation-models .is-selected').count(),1);
        assert.equal(await page.locator('.ec-tryon-generation-outfit .ec-tryon-generation-card').count(),4);
        const generationLayout = await page.locator('.ec-tryon-generation-references').evaluate(element => ({left:element.querySelector('.ec-tryon-generation-models').getBoundingClientRect().right,right:element.querySelector('.ec-tryon-generation-outfit').getBoundingClientRect().left}));
        assert.ok(generationLayout.left < generationLayout.right,'生成页模特应在左、服饰参考应在右');
