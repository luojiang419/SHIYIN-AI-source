...
(async () => {
    ...
    try {
        ...
        await page.locator('[data-tryon-wardrobe-role="tryon_extra_4"] [data-tryon-generation-description-role]').click();
        await page.locator('[data-tryon-wardrobe-role="tryon_extra_4"] .ec-tryon-generation-note-input').fill('深棕色皮鞋');
        await page.locator('[data-tryon-wardrobe-role="tryon_extra_4"] .ec-tryon-generation-note-input').press('Enter');
        await dropImage('.ec-tryon-generation-models');
        await page.locator('[data-tryon-generation-model-description="1"]').click();
        await page.locator('.ec-tryon-generation-models .ec-tryon-generation-note-input').fill('第二位模特');
        await page.locator('.ec-tryon-generation-models .ec-tryon-generation-note-input').press('Enter');
        ...
        assert.equal(await page.locator('.ec-tryon-generation-card input').count(),0,'生成页默认不显示描述编辑框');
        assert.match(await page.locator('.ec-tryon-generation-models .ec-tryon-generation-note').textContent(),/第二位模特/);
