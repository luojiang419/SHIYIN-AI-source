...
(async () => {
    ...
    try {
        ...
        assert.equal(await page.locator('.ec-tryon-generation-note').count(),0,'生成页不显示描述文字编辑控件');
        ...
        await page.waitForFunction(() => document.querySelector('#tryOnGuideStatus')?.textContent.includes('生成中') && document.querySelector('#generateButton:not(.submitting)'));
        assert.equal(tasks.length,2);
        const tryOn = tasks.find(task => task.operation === 'try_on');
        assert.equal(tryOn.inputs.find(item => item.role === 'source').instruction,'');
        ...
        assert.equal(tryOn.inputs.find(item => item.reference_id === 'tryon_extra_4').instruction,'');
