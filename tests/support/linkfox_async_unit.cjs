...
(async()=>{
    ...
    assert(node.linkfoxDeliveredTaskId.startsWith('canvas_video_linkfox_'));
    assert.equal(node.linkfoxTaskStatus,'视频已完成');
    ...
    const resumedNode={id:'resume',linkfoxTaskId:'canvas_video_saved',linkfoxTaskStatus:'视频正在生成中'};
    await resumed.api.generate(resumedNode,payload);
    assert.equal(resumed.calls.filter(c=>c[1]?.method==='POST').length,0);
    assert.equal(resumedNode.linkfoxTaskId,'');assert.equal(resumedNode.linkfoxDeliveredTaskId,'canvas_video_saved');
    assert.equal(resumedNode.linkfoxTaskStatus,'视频已完成');
    assert(resumed.calls.every(c=>c[1]?.cache==='no-store'));
