...
class BridgeLoopbackReceiveTests(unittest.TestCase):
    def test_direct_receive_uses_multipart_without_archive(self):
...
content = open(frame_path, "rb").read()
manifest = {
    "schema": "shiyin-film-bridge",
    "schema_version": 2,
    "bridge_id": "film:direct:board",
...
assert payload["frame_count"] == 1, payload
...
'''
            ...
            completed = subprocess.run(
                [sys.executable, "-c", code, str(frame)],
