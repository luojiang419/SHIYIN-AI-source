...
def test_image_quick_provider_and_model_use_cascading_drawer():
    prompt_start = JS.index("function imageQuickProviderItemsHtml(providerId)")
    ...
    assert 'data-image-quick-drawer-trigger="provider"' in choice_source
    assert 'data-image-quick-drawer-trigger="model"' in choice_source
    assert 'data-image-quick-drawer' in choice_source
    assert 'data-image-quick-drawer-level="provider"' in choice_source
    assert 'data-image-quick-drawer-level="model"' in choice_source
    assert "data-image-quick-provider-value" in choice_source
    assert "data-image-quick-model-value" in choice_source
    assert "function setImageQuickDrawerOpen" in bind_source
    assert "renderImageQuickModelDrawerItems" in bind_source
    assert "setImageQuickDrawerOpen(true, 'model')" in bind_source
    assert "querySelector('[data-image-quick-model-value]')?.focus({preventScroll:true})" in bind_source
    assert "setImageQuickDrawerOpen(false)" in bind_source
    assert ".image-quick-choice-drawer" in CSS
    assert ".image-quick-compose.image-quick-drawer-open .image-quick-choice-drawer" in CSS
    assert "grid-template-columns:minmax(0,.8fr) minmax(0,1.2fr)" in CSS
    assert ".image-node-prompt-panel {" in CSS
    assert "container-type:inline-size; overflow:visible;" in CSS
    assert "max-height:min(260px,calc(100vh - 96px))" in CSS
