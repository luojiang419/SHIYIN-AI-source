...
def test_native_directory_pickers_guard_empty_initial_path():
    ...
    assert source.count("$initialPath=[string]$env:SHIYIN_FOLDER_INITIAL_PATH;") == 4
    assert source.count("if($initialPath -and (Test-Path -LiteralPath $initialPath))") == 4
    assert "Test-Path -LiteralPath $args[0]" not in source
    assert source.count('"-Command", script],') == 4
    assert source.count('env=powershell_env,') == 4
