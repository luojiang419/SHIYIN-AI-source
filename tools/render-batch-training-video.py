...
for idx,s in enumerate(steps):
    base=Image.new('RGB',(W,H),'#f6eee4');d=ImageDraw.Draw(base)
    d.rectangle((0,0,W,105),fill='#332d28');d.text((55,27),'拾影  /  一键复刻 × 批量换款',font=font(29),fill='white')
    d.text((1200,35),'真实案例 · 操作教学',font=font(22),fill='#ead8c4')
    d.text((55,128),f'{idx+1:02d}  {s["title"]}',font=font(40),fill='#332d28')
    ...
    for j,src in enumerate(s['images']):
        ...
        base.paste(im,(x,y));d.text((55+j*(cell+20),792),s['labels'][j],font=font(19),fill='#766f68')
    wrap(d,s['text'],55,835,1490,font(26),'#58514b')
    d.text((55,955),'教学动画：真实素材与页面截图编排，非连续录屏，等待时间已压缩。',font=font(17),fill='#766f68')
    d.text((1390,955),f'{idx+1} / {len(steps)}',font=font(17),fill='#766f68')
    for frame in range(FPS*SECONDS):
        img=base.copy();q=ImageDraw.Draw(img);progress=(idx+frame/(FPS*SECONDS))/len(steps)
        q.rectangle((0,992,int(W*progress),999),fill='#9a6636')
        if n==1:
            px=1060;py=650;radius=18+int(6*math.sin(frame/FPS*4))
            q.ellipse((px-radius,py-radius,px+radius,py+radius),outline='#9a6636',width=4)
            q.polygon([(px,py),(px+4,py+29),(px+11,py+20),(px+24,py+19)],fill='#fff',outline='#332d28')
