...
3. “模型推理尺寸”只决定神经网络内部计算量，不是导出分辨率。1080p 视频可保持 1920×1080 输出，同时继续使用 392 或 322 做模型内部推理。
4. 默认使用“原始 FPS + 全部帧 + 原始分辨率”，不会主动抽帧、截断或缩小输出；也可选择 6–120 FPS、限定帧数、720p/1080p/1440p/4K 档位。
5. 文件载入后会显示宽高、FPS、时长和总帧数；左侧右上角可随时“更换视频”或“移除视频”。
6. 点击“开始模型测试”。输出目录包含归一化 Relative Depth、H.264 灰度视频和运行元数据。
7. 调整黑白点、中间层次、对比度、亮度、平滑或反转；界面先给出近似滤镜预览，点击“应用参数”后从原始 Relative Depth 精确重算，不重复运行模型。
8. 可导出当前深度视频和版本化 JSON 参数配置，也可重新导入配置。

统一 worker 将推理随机种子固定为 0。GemDepth 上游即使在 eval 模式仍会使用随机 pose mask，固定 seed 可保证模型与参数对比能够复现。

默认现在是完整提取模式。原始 FPS 模式不经过 FFmpeg `fps` 滤镜，全部帧模式不传递 `-frames:v` 截断参数；原始分辨率模式保持输入宽高（编码要求为偶数时至多修正 1 像素）。

完整长视频的耗时、RAM 和临时磁盘占用会随帧数线性增加，可使用界面的“停止当前任务”安全终止。GemDepth 模型推理尺寸默认 392；VDA Base 默认 322。518 及更高模型推理尺寸在 Windows 无 FlashAttention 的 PyTorch 上可能占满 16GB 显存并显著变慢，只用于压力验证。
