...
function Get-Verified($Package, [string]$Destination) {
    if (Test-Path -LiteralPath $Destination) {
        if ((Get-Item -LiteralPath $Destination).Length -eq $Package.size -and (Get-FileHash -LiteralPath $Destination -Algorithm SHA256).Hash -eq $Package.sha256) { return }
    ...
    if ((Get-Item -LiteralPath $partial).Length -ne $Package.size -or (Get-FileHash -LiteralPath $partial -Algorithm SHA256).Hash -ne $Package.sha256) {
...
if (!(Test-Path -LiteralPath $worker) -and !(Test-Path -LiteralPath $devPython)) {
    ...
    if ($variant.assemble_archive) {
        ...
        if ((Get-Item -LiteralPath $joined).Length -ne $variant.assemble_archive.size -or (Get-FileHash -LiteralPath $joined -Algorithm SHA256).Hash -ne $variant.assemble_archive.sha256) { throw 'Assembled runtime integrity failure' }
        $archives = @($joined)
    }
...
if (Test-Path -LiteralPath $worker) {
    ...
    $overlayManifest = Get-Content -LiteralPath (Join-Path $Root 'worker-overlays/manifest.json') -Raw | ConvertFrom-Json
    ...
    if ((Get-FileHash -LiteralPath $overlay -Algorithm SHA256).Hash -ne $overlayEntry.sha256) { throw 'Worker overlay integrity failure' }
    if ((Get-FileHash -LiteralPath $worker -Algorithm SHA256).Hash -ne $overlayEntry.sha256) {
...
$models = Get-Content -LiteralPath (Join-Path $Root 'model-download-manifest.json') -Raw | ConvertFrom-Json
