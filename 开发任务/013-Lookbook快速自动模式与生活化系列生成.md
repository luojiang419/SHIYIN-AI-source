...
当前任务已完成。此前的单一“参考图真实感”预设已在后续任务 014 中替换为按多人、单人、运动、休闲等创作类型组织的新预设。
...
- [x] 保留有需求时联网搜索与创意策划链路
- [x] 完成测试、文档和 Git 提交
- [x] 新增参考图驱动与多人物互动/场景融合提示词约束（已由任务 014 的类型预设承接）
...
- 最近 Git commit：`b64d0ce fix: preserve selected lookbook style defaults`；功能实现 commit：`72a1d90 feat: add reference-first naturalism lookbook skill`（均已推送 `origin/feat/storyboard-merge-node`）
...
- 旧的单一 `reference-first-naturalism` Skill 与卡片已由任务 014 的类型化预设替换。
