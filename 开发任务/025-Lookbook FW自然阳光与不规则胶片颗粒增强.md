...
用户反馈上一版 FW Lookbook 已接近目标，但光线仍不够自然通透，胶片颗粒缺少粗粝感且呈规则/均匀噪点。现已把“自然阳光”从色彩描述提升为可执行的曝光与光照关系，并增加不规则、分尺度、随亮度变化的模拟胶片颗粒。后续复测又发现人物发色漂移，已将无限画布人物端口改为 `subject` 语义、场景端口改为 `scene` 语义，并完成对照验收报告。
...
- [x] 增加回归测试并生成样本
- [x] 更新文档、提交并推送
- [x] 对照视频帧与生成图完成黑白点、颗粒、阳光和情绪动作验收报告
