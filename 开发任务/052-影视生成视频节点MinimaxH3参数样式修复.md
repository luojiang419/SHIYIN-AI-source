...
已完成修复：影视制作组的 `film-video` 节点现在依据 `MiniMax H3` 模型切换到 H3 专用分辨率、无上下限采样步数和参考模式设置；绑定器、经典画布和智能画布提交链路均已同步。后续已移除采样步数原有的 4–30 限制，用户输入值会直接透传给 H3 服务。
...
- 单元测试：H3/视频/影视相关 57 passed（含采样步数上下界解除回归）；完整影视集合另有 1 项既有契约失败
...
- `film-video` 选择 MiniMax H3 后显示 H3 分辨率预设、无 4–30 范围限制的采样步数及互斥参考模式按钮。
...
- 2026-09-06：移除 MiniMax H3 采样步数的 4–30 前后端限制，补充超范围透传测试。
