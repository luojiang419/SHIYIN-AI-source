...
分支 feat/film-workflow-canvas。保留所有既有未提交修改，增量基线在系统临时目录 shiyin-linkfox-unified-1dl6rgaq。

## 下一步

1. 完成增量审查、独立暂存、提交与正常 push。
2. 源码运行即可使用；本次不构建安装包，不发起真实付费视频任务。
3. 成片质量需要用户提供案例后进行 A/B 审片，不能由提示词规范或模拟测试保证。
